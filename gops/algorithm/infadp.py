        ...
        with torch.no_grad():
            for step in range(self.forward_step):
                if step == 0:
                    ...
                    v_target =  (-r) * self.reward_scale
                else:
                    o = o2
                    a = self.networks.policy(o)
                    o2, r, d, info = self.envmodel.forward(o, a, d, info)
                    v_target +=  self.gamma ** step * (-r) * self.reward_scale

            v_target += (
                     self.gamma ** self.forward_step * self.networks.v_target(o2)
            )
        loss_v = ((v - v_target) ** 2).mean()

        if self.equilibrium is not None:
            loss_v += self.networks.v(torch.tensor(self.equilibrium, dtype=torch.float32).reshape(1, -1)).squeeze() ** 2
