...
class INFDHP(AlgorithmBase):
    ...
    def adjustable_parameters(self):
        para_tuple = ("gamma", "tau", "pev_step", "pim_step", "forward_step", "reward_scale", "equilibrium")
        return para_tuple
    ...
    def __compute_loss_v(self, data):
        ...
        o_list, a_list, r_list, o2_list, d_list = [], [], [], [], []
        o_init = deepcopy(o)
        costate = self.networks.v(o_init)
        info = data
        ...
        lambda_n = self.networks.v_target(o2).detach()
        fake_Hamiltonian += self.gamma ** self.forward_step * torch.sum(o2 * lambda_n, dim=1)
        costate_target = torch.autograd.grad(fake_Hamiltonian.sum(), o_init)[0].detach()

        costate_error = (costate - costate_target)
        ...
        if self.equilibrium is not None:
            ...
            loss_costate += (self.networks.v(equilibrium) ** 2).mean()

        return loss_costate, torch.mean(costate)
    ...
    def __compute_loss_policy(self, data):
        ...
        lambda_n = self.networks.v(o2).detach()
        fake_Hamiltonian += self.gamma ** self.forward_step * torch.sum(o2 * lambda_n, dim=1)
