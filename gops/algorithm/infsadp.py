...
class INFSADP(AlgorithmBase):
    ...
    def __compute_loss_v(self, data):
        ...
        costate = torch.autograd.grad(v.sum(), o_init, create_graph=True)[0]  # create_graph=True makes it able to compute 2-order gradient
        info = data
        backup = 0
        ...
        v_target = backup + self.gamma ** self.forward_step * self.networks.v_target(o2)
        ...
        if self.equilibrium is not None:
            ...
            loss_v += self.networks.v(equilibrium).squeeze() ** 2

        loss_costate = torch.zeros(1)
        loss_ratio = 0
        if self.w_loss_v == 1:
            loss_critic = loss_v
        else:
            lambda_n = torch.autograd.grad(self.networks.v_target(o2).sum(), o2)[0].detach()
            ...
            if self.equilibrium is not None:
                ...
                loss_costate += (torch.autograd.grad(self.networks.v(equilibrium).sum(), equilibrium, create_graph=True)[0] ** 2).mean()
            if self.w_loss_v == 0:
                loss_critic = loss_costate
            else:
    ...
    def __compute_loss_policy(self, data):
        ...
        for step in range(self.forward_step):
            if step == 0:
                ...
                v_pi =  (-r) * self.reward_scale
            else:
                o = o2
                a = self.networks.policy(o)
                o2, r, d, info = self.envmodel.forward(o, a, d, info)
                v_pi +=  self.gamma ** step * (-r) * self.reward_scale
