...
from gops.env.env_ocp.pyth_robotarm_model import Dynamics

gym.logger.setLevel(gym.logger.ERROR)
plt.rcParams["toolbar"] = "None"


class PythRobotArm(PythBaseEnv):
    metadata = {
    ...
    def __init__(self, **kwargs):
        ...
        if work_space is None:
            # initial range of # [q1, dq1, q2, dq2]
            init_high = np.array([1, 1, 1, 1], dtype=np.float32)
            init_low = np.array([-1, -1, -1, -1], dtype=np.float32)
            work_space = np.stack((init_low, init_high))
        super(PythRobotArm, self).__init__(
        ...
        self.observation_space = spaces.Box(
            low=np.array([-np.pi, -10, -np.pi, -10], dtype=np.float32),
            high=np.array([np.pi, 10, np.pi, 10], dtype=np.float32),
        )

        # define action space here
        lb_action = [-1, -1]  # [torque1, torque2]
        hb_action = [1, 1]
    ...
    def step(self, action: np.ndarray, adv_action=None):
        ...
        for _ in range(self.discrete_num):
            next_obs_batch = self.dynamics.f_xu(
                obs_batch, act_batch, self.dt / self.discrete_num
            )
...
def env_creator(**kwargs):
    """
    make env `pyth_robotarm`
    """
    return PythRobotArm(**kwargs)
