...
class LQDynamics:
    ...
    def prediction_forward_discrete(self,
        ...
        tmp = (torch.mm(self.A, x_t.T) + torch.mm(self.B, u_t.T)) * self.time_step + x_t.T
    ...
    def prediction_backward_discrete(self,
                   x_t: Union[torch.Tensor, np.ndarray], u_t: Union[torch.Tensor, np.ndarray]
                   ) -> Union[torch.Tensor, np.ndarray]:
        numpy_flag = False
        if isinstance(x_t, np.ndarray):
...
class LqEnv(PythBaseEnv):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.discrete_mode = config["discrete_mode"]
    ...
    def step(self, action: np.ndarray, adv_action=None):
        ...
        if self.discrete_mode == "forward_discrete":
            self.obs = self.dynamics.prediction_forward_discrete(self.obs, action)
        elif self.discrete_mode == "backward_discrete":
            self.obs = self.dynamics.prediction_backward_discrete(self.obs, action)
        else:
            raise ValueError
...
class LqModel(PythBaseModel):
    ...
    def forward(self, obs: torch.Tensor, action: torch.Tensor, done: torch.Tensor, info: InfoDict) \
        ...
        if self.discrete_mode == "forward_discrete":
            next_obs = self.dynamics.prediction_forward_discrete(obs, action)
        elif self.discrete_mode == "backward_discrete":
            next_obs = self.dynamics.prediction_backward_discrete(obs, action)
        else:
            raise ValueError
